...
void expect_invalid(const std::string& row)
{
    try
    {
        static_cast<void>(tick_csv::parse_row(row));
...
int main()
{
    const Tick tick = tick_csv::parse_row("42,EURUSD,1.2345,100,b");
    assert(tick.ts == 42);
    ...
    assert(tick_csv::parse_row("43,EURUSD,1.2346,200,").side == Side::Unknown);
    assert(tick_csv::parse_row("43,EURUSD,1.2346,200").side == Side::Unknown);
    expect_invalid("43,EURUSD,not-a-price,200,B");
    expect_invalid("43,EURUSD,1.2346");
    assert(tick_csv::parse_row("43,EURUSD,1.2346,200,B,extra").side == Side::Buy);
}